...
function Servis(props) {
  ...
  return (
    <div className="min-h-[640px] w-[100%]  flex gap-7">
      <div className="flex flex-col justify-start align-center content-center border-r border-[#302F2F] w-[30%]  h-[100%]">
        {navbarServis.map((item1) => {
          return (
            <div
              key={item1.id}
              onClick={() => (setClick(item1.id), setName(item1.title))}
              className={`w-[100%] h-[75px] border-b border-[#302F2F] cursor-pointer flex flex-col justify-center content-start align-center   `}
            >
              <p
